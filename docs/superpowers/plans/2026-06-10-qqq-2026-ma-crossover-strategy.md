# QQQ 2026 MA-Crossover Strategy Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [x]`) syntax for tracking.

**Goal:** Build `qqq_2026_strategy.py`, a standalone backtest comparing an untuned 10/20 MA-crossover baseline and a Sharpe-maximizing grid-searched variant against QQQ buy-and-hold over 2026.

**Architecture:** One flat script (matches repo convention, e.g. `tusharStrategyDev.py`). Pure functions: `load_data` â†’ `backtest` â†’ `metrics` â†’ `grid_search` â†’ `report`/`plot`. Causality enforced by `position.shift(1)`; sanity assertions run on every execution. No database, no separate test file (per approved spec â€” this is a one-shot research tool).

**Tech Stack:** Python, pandas, numpy, matplotlib (all already required by this repo).

**Spec:** `docs/superpowers/specs/2026-06-10-qqq-2026-ma-crossover-strategy-design.md`

---

### Task 1: Engine â€” data loading, backtest, metrics, sanity checks

**Files:**
- Create: `qqq_2026_strategy.py`

- [x] **Step 1: Create the script with loader, engine, metrics, and sanity checks**
...
- [x] **Step 2: Run the script to verify load + sanity checks**

Run: `python qqq_2026_strategy.py`
Expected output:
```
Loaded 109 rows: 2026-01-02 to 2026-06-09
...
```bash
git add qqq_2026_strategy.py
git commit -m "Add QQQ 2026 strategy engine: loader, MA-crossover backtest, sanity checks"
```

---

### Task 2: Baseline, grid search, and console report

**Files:**
- Modify: `qqq_2026_strategy.py` (extend Task 1 file)

- [x] **Step 1: Add grid search and report functions, wire into main**

Add after `sanity_checks()` (before `main`):
...
- [x] **Step 2: Run and verify the report**

Run: `python qqq_2026_strategy.py`
Expected: sanity checks pass, then a 3-row comparison table (Buy & hold row shows total â‰ˆ 16.75%) and a 5-row top-configs table. No assertion errors.

- [x] **Step 3: Commit**

```bash
git add qqq_2026_strategy.py
git commit -m "Add baseline, Sharpe grid search, and console report to QQQ 2026 strategy"
```

---

### Task 3: Equity-curve chart

**Files:**
- Modify: `qqq_2026_strategy.py` (extend Task 2 file)

- [x] **Step 1: Add plotting function and wire into main**
...
- [x] **Step 2: Run full script and verify the PNG**

Run: `python qqq_2026_strategy.py`
Expected: full report plus `Equity curves saved to qqq_2026_strategy_results.png`; file exists and is non-trivial (>20 KB).

- [x] **Step 3: Commit**

```bash
git add qqq_2026_strategy.py
git commit -m "Add equity-curve chart output to QQQ 2026 strategy"
```

(`qqq_2026_strategy_results.png` is a generated artifact â€” do not commit it; `charts/` convention in this repo keeps generated output untracked.)
