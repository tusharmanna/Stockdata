...
# Run the scanner (finds stocks matching all three criteria — see Scanner section)
python scanner.py
```
...
# yfinance, pandas, requests — sqlite3 is stdlib
```

## Architecture

All data flows in one direction: **download → store → compute → scan**.
...
ticker_list.py   →  downloader.py  →  database.py (prices table)
                                    ...
                                    indicators.py  →  database.py (indicators table)
                                           ↓
                                     scanner.py    →  database.py (scan_results table)
```
...
**`database.py`** — thin SQLite wrapper (`stockdata.db` in project root). WAL mode enabled. All functions open and close their own connection. Three tables: `prices`, `indicators`, `scan_results`.
...
**`scanner.py`** — runs a single SQL query (two CTEs) against the DB to find stocks where, on the latest trading day: close is above all four MAs, volume is below `vol_avg30`, and the candle is an inside bar (high < prev_day_high, low > prev_day_low). Saves matches to `scan_results` keyed by `(scan_date, ticker)`.
...
scan_results (scan_date, ticker PK) — scanner output with snapshot of all relevant fields
```

Join `prices` and `indicators` on `(ticker, date)`. `scan_results` stores a denormalized snapshot so past scan results are self-contained.

## Automation (Windows Task Scheduler)

`run_daily.bat` runs `main.py` then `scanner.py` every weekday at 8 PM ET and appends output to `logs\YYYY-MM-DD.log`.
